...
export default async function CreateEvent (req, res) {
    ...
    console.log(JSON.stringify(category), user)
     let query = `INSERT INTO events (title, description, event_date, event_time, duration, location, image_url, category, max_attendees, ticket_price, is_featured, current_attendees, user) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    try{
